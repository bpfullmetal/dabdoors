 ...
 * @return void
 */
function hootubix_tgmpa_plugins() {
	$plugins = array();

	$wpv = get_bloginfo( 'version' );
	if ( version_compare( $wpv, '5.8', '>=' ) ) {
		$plugins[] = array(
			'name'     => __( 'Classic Widgets', 'hoot-ubix' ),
			'slug'     => 'classic-widgets',
			'required' => false,
		);
	}
	$plugins = apply_filters( 'hootubix_tgmpa_plugins', $plugins );
...
// add_filter( 'tgmpa_register', 'hootubix_tgmpa_plugins' );