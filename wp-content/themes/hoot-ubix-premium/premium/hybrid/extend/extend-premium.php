...
if ( !class_exists( 'Hybrid_Extend_Premium' ) ) {
	class Hybrid_Extend_Premium {
		...
		function constants() {
			...
			if ( is_child_theme() )
				$template_name = $hybridextend_theme->parent()->get( 'Name' );
			else
				$template_name = HYBRIDEXTEND_THEME_NAME;
			$template_name = preg_replace( '/ ?child/i', '', $template_name ); // instead of '/_?child$/'
			define( 'HYBRIDEXTEND_TEMPLATE', preg_replace( '/ ?premium/i', '', $template_name ) ); // instead of '/_?premium$/'
		...
		function extensions() {
			...
			require_if_theme_supports( 'custom-404', HYBRIDEXTEND_PREMIUM_DIR . 'extensions/custom-404.php' );
