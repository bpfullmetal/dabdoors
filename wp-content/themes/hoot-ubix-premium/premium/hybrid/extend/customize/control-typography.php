			...
			if( $size || $face || $style || $color ):
				...
				if ( $style ):
					...
					$value[ "{$key}-style" ] = array(
						...
						'description' => array(
							'type' => 'blue',
							'text' => sprintf( __( 'NOTE: Not all fonts support all styles/variants. For example, "Playball" font does not support Bold/Italic/Light variant.<hr />For more info, check the %1$sGoogle Fonts Library%2$s', 'hybrid-core' ), '<a href="https://www.google.com/fonts">', '</a>'),
						),
					);
