...
class Hoot_Options_Interface {
	...
	static function hootoptions_field( $prefix, $group_name='', $field, $val, $echo=true ) {
		...
		case "icon":
			...
			$output .= '<div id="' . esc_attr( $field_id_prefix . $field['id'] . '-icon-picked' ) . '" class="hoot-of-icon-picked"><i class="fa ' . esc_attr( $val ) . '"></i><span>' . __( 'Select Icon', 'hybrid-core' ) . '</span></div>';

			$output .= '<div id="' . esc_attr( $field_id_prefix . $field['id'] . '-icon-picker-box' ) . '" class="hoot-of-icon-picker-box">';

				$output .= '<div class="hoot-of-icon-picker-list"><i class="fa fa-ban hoot-of-icon-none" data-value="0" data-category=""><span>' . __( 'Remove Icon', 'hybrid-core' ) . '</span></i></div>';
