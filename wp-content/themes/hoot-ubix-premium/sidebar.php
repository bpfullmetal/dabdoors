	...
	<aside <?php hybridextend_attr( 'sidebar', 'primary' ); ?>>
		...
		elseif ( current_user_can( 'edit_theme_options' ) ) : // If the sidebar has no widgets.
			...
			the_widget(
				'WP_Widget_Text',
				array(
					'title'  => __( 'Example Widget', 'hoot-ubix-premium' ),
					/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
					'text'   => sprintf( __( 'This is an example widget to show how the Primary sidebar looks by default. You can add custom widgets from the %1$swidgets screen%2$s in wp-admin.', 'hoot-ubix-premium' ), '<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '">', '</a>' ),
					'filter' => true,
				),
				array(
			...
			elseif ( current_user_can( 'edit_theme_options' ) ) : // If the sidebar has no widgets.
				...
				the_widget(
					...
					array(
						'title'  => __( 'Example Widget', 'hoot-ubix-premium' ),
						/* Translators: The %s are placeholders for HTML, so the order can't be changed. */
						'text'   => sprintf( __( 'This is an example widget to show how the Secondary sidebar looks by default. You can add custom widgets from the %1$swidgets screen%2$s in wp-admin.', 'hoot-ubix-premium' ), '<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '">', '</a>' ),
						'filter' => true,
					),
					array(
						'before_widget' => '<section class="widget widget_text">',
