...
function hoot_update_browser() {
	$notice = '<!--[if lte IE 9]><p class="chromeframe">' .
			  sprintf( __( 'You are using an outdated browser (IE 8 or before). For a better user experience, we recommend %1$1supgrading your browser today%2$2s or %3$3sinstalling Google Chrome Frame%4$4s', 'hoot-ubix-premium' ), '<a href="http://browsehappy.com/">', '</a>', '<a href="http://www.google.com/chromeframe/?redirect=true">', '</a>' ) .
			  '</p><![endif]-->';
...
function hoot_meta_info_blocks( $args = '', $context = '' ) {
	...
	<div class="entry-byline">
		...
		if ( !empty( $display['author'] ) ) :
			$blocks['author']['label'] = __( 'By:', 'hoot-ubix-premium' );
			ob_start();
		...
		if ( !empty( $display['date'] ) ) :
			$blocks['date']['label'] = __( 'On:', 'hoot-ubix-premium' );
		...
		if ( !empty( $display['cats'] ) ) :
			...
			if ( !empty( $category_list ) ) :
				$blocks['cats']['label'] = __( 'In:', 'hoot-ubix-premium' );
				$blocks['cats']['content'] = $category_list;
			endif;
		endif;

		if ( !empty( $display['tags'] ) && get_the_tags() ) :
			$blocks['tags']['label'] = __( 'Tagged:', 'hoot-ubix-premium' );
			$blocks['tags']['content'] = ( ! get_the_tags() ) ? __( 'No Tags', 'hoot-ubix-premium' ) : get_the_tag_list( '', ', ', '' );
		endif;

		if ( !empty( $display['comments'] ) && comments_open() ) :
			$blocks['comments']['label'] = __( 'With:', 'hoot-ubix-premium' );
			ob_start();
		...
		if ( $edit_link = get_edit_post_link() ) :
			...
			$blocks['editlink']['content'] = '<a href="' . $edit_link . '">' . __( 'Edit This', 'hoot-ubix-premium' ) . '</a>';
		endif;
